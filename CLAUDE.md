...
- `src/code_reviewer/reviewers/`: reviewer backends and reconciliation (`claude_sdk`, `codex_cli`, `codex_agents_sdk`, `antigravity_cli`, `opencode_cli`, `triage`, `lightweight`, `reconcile`)
...
- All model interactions go through CLI/SDK runners (`_run_claude_prompt`, `run_codex_prompt`, `run_antigravity_prompt`, `run_opencode_prompt`) — no direct API calls
...
- Backend functions support claude/codex/antigravity/opencode with graceful fallback on errors
